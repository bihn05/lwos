...
// standard US QWERTY keymap, indexed by scancode
static const keymap_entry_t keymap[] = {
    ...
    // null entry for unsupported scancodes
    {0x00, 0, 0, 0, 0}
};
...
static const extended_key_t extended_keymap[] = {
    ...
    // Add more extended keys as needed
    {0x1C, 0x9C, "KP_Enter",   '\n', '\n'}, 
    {0x35, 0xB5, "KP_Slash",   '/', '/'},  
    // Add more keypad keys as needed
...
#define KEY_EXTENDED_PREFIX2   0xE1  // Pause key uses 0xE1 and has a unique sequence, but we can treat it as a second extended prefix for simplicity
#define KEY_RELEASE_PREFIX     0xF0  // 假设我们使用0xF0作为释放码的前缀，虽然实际键盘可能不使用这个值，但它可以帮助我们区分按下和释放事件

// 定义一些特殊键的ASCII码值，方便在处理时使用
