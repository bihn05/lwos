...
int bbitmap_find_top(bbitmap_t* map, int value);

static int bitmap_test(bitmap_t* map, int index);
void bitmap_set(bitmap_t* map, int index, int value);
int bitmap_find_continuous(bitmap_t* map, int size);
static int bitmap_find_top(bitmap_t* map, int value);
...
static int bitmap_test(bitmap_t* map, int index) {
    ...
    int bit_offset = index & 7;
    uint8_t mask = 1 << bit_offset;
    uint8_t tmp = map->bits[byte_index];
    tmp &= mask;
...
void bitmap_set(bitmap_t* map, int index, int value) {
    ...
    uint8_t mask = ~(1 << bit_offset);
    uint8_t new_value = (value & 1) << bit_offset;

    map->bits[byte_index] = (map->bits[byte_index] & mask) | new_value;
}
static int bitmap_find_top(bitmap_t* map, int value) {
    for (int i = 0; i < 256; i++) {
        if (bitmap_test(map, i) == value) {
            return i;
        }
    }
    return -1;
